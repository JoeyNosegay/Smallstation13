...
/datum/emergency_shuttle_controller/proc/shuttle_arrived()
	if (!shuttle.location)	//at station
		if (autopilot)
			...
			if (evac)
				emergency_shuttle_docked.Announce("The Evacuation is now beginning. You have approximately [round(estimate_launch_time()/60,1)] minutes to board an Escape Pod.")
			else
				priority_announcement.Announce("Our scheduled [dock_name] has been prepared. We will depart in approximately [round(emergency_shuttle.estimate_launch_time()/60,1)] minutes.")
...
/datum/emergency_shuttle_controller/proc/call_evac()
	...
	evac = 1
	emergency_shuttle_called.Announce("An emergency evacuation is underway. Gather your belongings and report to an escape pod in [round(estimate_arrival_time()/60)] minutes.")
...
/datum/emergency_shuttle_controller/proc/call_transfer()
	...
	priority_announcement.Announce("A [dock_name] has been scheduled. The station is warming up. We will leave orbit in approximately [round(estimate_arrival_time()/60)] minutes.")
...
/datum/emergency_shuttle_controller/proc/recall()
	...
	if (evac)
		emergency_shuttle_recalled.Announce("The evacuation has been cancelled.")
		...
		priority_announcement.Announce("The scheduled bluespace jump has been terminated.")
