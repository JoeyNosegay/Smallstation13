		...
		if(H.mind)
			...
			switch(rank)
				...
				if("Expedition Head")
					...
					captain_announcement.Announce("The vote is in, [H.real_name] is Head Lead this week!", new_sound=announce_sound)
...
/datum/controller/occupations/proc/LateSpawn(var/mob/living/carbon/human/H, var/rank)
	...
	if(spawnpos && istype(spawnpos))
		if(spawnpos.check_job_spawning(rank))
			...
			H << "Your chosen spawnpoint ([spawnpos.display_name]) is unavailable for your chosen job. Waking you in the dorms instead."
			H.loc = pick(latejoin)
