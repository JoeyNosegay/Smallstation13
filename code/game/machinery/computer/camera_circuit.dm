...
//when adding a new camera network, you should only need to update these two procs
	New()
		...
		possibleNets["SS13"] = access_coms
		possibleNets["Mining"] = access_cargo
		possibleNets["Cargo"] = access_cargo
		possibleNets["Research"] = access_science
		possibleNets["Medbay"] = access_medical
		..()
