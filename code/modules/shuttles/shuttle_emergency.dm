...
/datum/shuttle/ferry/emergency/move(var/area/origin,var/area/destination)
	...
	if (origin == area_station)	//leaving the station
		...
		if (emergency_shuttle.evac)
			priority_announcement.Announce("The Evacuation has taken place. Estimate [round(emergency_shuttle.estimate_arrival_time()/60,1)] minutes until the shuttles are retrieved.")
		else
			priority_announcement.Announce("The bluespace jump is in process. Estimate [round(emergency_shuttle.estimate_arrival_time()/60,1)] minutes until the shuttle halts.")
